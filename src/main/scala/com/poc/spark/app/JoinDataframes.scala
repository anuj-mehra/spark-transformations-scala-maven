...
object JoinDataframes extends App with Serializable{

 val sparkSession = SparkSessionConfig("JoinDataframes", true).getSparkSession

 val processor  = new JoinDataframes
 processor join3 sparkSession

}

class JoinDataframes extends Serializable {

 def join1:(SparkSession) => Unit = (spark: SparkSession) => {
 ...
 def join2:(SparkSession) => Unit = (spark: SparkSession) => {
  ...
  import org.apache.spark.sql.functions._
  val joinedDf = df1.join(broadcast(df2), Seq("deptId"), "inner")
  joinedDf.show(false)

 }

 def join3:(SparkSession) => Unit = (spark: SparkSession) => {
  ...
  val joinedDf = df1.join(df2, df1("deptId") === df2("deptId"),"rightouter")
  joinedDf.show(false)
 }
